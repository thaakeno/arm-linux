...
val officialProrootSha256 = linkedMapOf(
...
val prepareOfficialProrootRuntime by tasks.registering {
    ...
    val destination = file("src/main/jniLibs/arm64-v8a")
    outputs.files(officialProrootSha256.keys.map { destination.resolve(it) })
    doLast {
        destination.mkdirs()
        ...
        officialProrootSha256.forEach { (name, expected) ->
