...
object VesselVirtioInput {
    ...
    @Volatile private var crosvmMode = false

    @JvmStatic private external fun nativeConfigure(touch: String, pointer: String, keyboard: String)
    @JvmStatic private external fun nativeCrosvmConfigure(touch: String, pointer: String, keyboard: String): Boolean
    @JvmStatic private external fun nativeCrosvmAbsolute(x: Int, y: Int, down: Boolean): Boolean
    @JvmStatic private external fun nativeCrosvmRelative(dx: Int, dy: Int): Boolean
    @JvmStatic private external fun nativeCrosvmButton(code: Int, down: Boolean): Boolean
    @JvmStatic private external fun nativeCrosvmScroll(x: Int, y: Int): Boolean
    @JvmStatic private external fun nativeCrosvmKey(code: Int, down: Boolean): Boolean
    @JvmStatic private external fun nativeCrosvmStatus(): String
    ...
    fun configure(touch: String, pointer: String, keyboard: String) {
        crosvmMode = false
        nativeConfigure(touch, pointer, keyboard)
    }

    fun configureCrosvm(touch: String, pointer: String, keyboard: String): Boolean {
        crosvmMode = true
        return nativeCrosvmConfigure(touch, pointer, keyboard)
    }

    fun send(type: String, values: Map<String, Any>): Boolean = when (type) {
        "abs" -> if (crosvmMode) nativeCrosvmAbsolute(values.int("x"), values.int("y"), values.bool("down")) else nativeAbsolute(values.int("x"), values.int("y"), values.bool("down"))
        "rel" -> if (crosvmMode) nativeCrosvmRelative(values.int("dx"), values.int("dy")) else nativeRelative(values.int("dx"), values.int("dy"))
        "btn" -> if (crosvmMode) nativeCrosvmButton(values.int("code"), values.bool("down")) else nativeButton(values.int("code"), values.bool("down"))
        "scroll" -> if (crosvmMode) nativeCrosvmScroll(values.int("x"), values.int("y")) else nativeScroll(values.int("x"), values.int("y"))
        "key" -> if (crosvmMode) nativeCrosvmKey(values.int("code"), values.bool("down")) else nativeKey(values.int("code"), values.bool("down"))
        else -> false
    }

    fun absoluteNormalized(x: Float, y: Float, down: Boolean): Boolean {
        val ix = (x.coerceIn(0f, 1f) * 32767f).toInt()
        val iy = (y.coerceIn(0f, 1f) * 32767f).toInt()
        return if (crosvmMode) nativeCrosvmAbsolute(ix, iy, down) else nativeAbsolute(ix, iy, down)
    }
    ...
    fun relative(dx: Float, dy: Float): Boolean {
        ...
        return (ix == 0 && iy == 0) || if (crosvmMode) nativeCrosvmRelative(ix, iy) else nativeRelative(ix, iy)
    }

    fun button(code: Int, down: Boolean): Boolean = if (crosvmMode) nativeCrosvmButton(code, down) else nativeButton(code, down)
    fun key(code: Int, down: Boolean): Boolean = if (crosvmMode) nativeCrosvmKey(code, down) else nativeKey(code, down)
    ...
    fun scrollPrecise(x: Float, y: Float): Boolean {
        ...
        return (ix == 0 && iy == 0) || if (crosvmMode) nativeCrosvmScroll(ix, iy) else nativeScroll(ix, iy)
    }
    ...
    fun status(): String = runCatching { if (crosvmMode) nativeCrosvmStatus() else nativeStatus() }.getOrElse { "virtio-input-error:${it.message}" }
