...
data class SessionState(
    ...
    val debianProtected:Boolean=false,
...
class VmSessionService : Service() {
    ...
    private val connection=object:ServiceConnection {
        ...
        override fun onServiceDisconnected(name:ComponentName) {
            ...
            state.value=state.value.copy(
                connected=false,
                running=false,
                message="Shizuku bridge disconnected; VM files retained"
            )
        }
    }
    ...
    override fun onCreate() {
        ...
        startForeground(
            ...
            Notification.Builder(this,"vm")
                ...
                .setContentText("AVF Linux session")
                ...
                withContext(Dispatchers.IO) {
                    File(filesDir,"verification-runtime.json").writeText(
                        JSONObject()
                            ...
                            .put("debian_installed",current.debianInstalled)
                            .put("debian_boot",current.debianBoot)
                            .put("debian_identity",current.debianIdentity)
                            .put("debian_protected",current.debianProtected)
                            .put("display",current.display)
                            .put("gles",current.graphics)
                            .put("kde",if(current.kdeInstalled)"PASS" else current.kdeStage)
    ...
    private fun applyStatus(raw:String) {
        ...
        val installError=o.optString("installError")
        val kdeError=o.optString("kdeError")
        val running=o.optBoolean("running")
        val failure=o.optString("failureStage","none")

        var message=state.value.message
        if(kdeError.isNotBlank()) message=kdeError
        else if(installError.isNotBlank()) message=installError
        else if(!running && failure=="none" && message.contains("Managed VM is not running",true)) {
            message="AVF bridge ready"
        }

        state.value=state.value.copy(
            running=running,
            name=o.optString("name"),
            mode=o.optString("mode","none"),
            console=o.optString("log"),
            vmApiInit=o.optString("vmApiInit",state.value.vmApiInit),
            vmDataDir=o.optString("vmDataDir",state.value.vmDataDir),
            vmCreation=o.optString("vmCreation",state.value.vmCreation),
            vmBoot=o.optString("vmBoot",state.value.vmBoot),
            connectVsock=o.optString("connectVsock",state.value.connectVsock),
            vsockFdReceived=o.optString("vsockFdReceived",state.value.vsockFdReceived),
            adbHandshake=o.optString("adbHandshake",state.value.adbHandshake),
            guestCommand=o.optString("guestCommand",state.value.guestCommand),
            failureStage=failure,
            capabilities=o.optString("capabilities",state.value.capabilities),
            debianInstalled=o.optBoolean("debianInstalled"),
            debianInstalling=o.optBoolean("debianInstalling"),
            installProgress=o.optDouble("installProgress",-1.0),
            installBytes=o.optLong("installBytes"),
            installTotal=o.optLong("installTotal",-1),
            installError=installError,
            debianBoot=o.optString("debianBoot",state.value.debianBoot),
            debianIdentity=o.optString("debianIdentity",state.value.debianIdentity),
            debianProtected=o.optBoolean("debianProtected"),
            display=o.optString("display",state.value.display),
            graphics=o.optString("guestGraphics",state.value.graphics),
            kdeInstalled=o.optBoolean("kdeInstalled"),
            kdeInstalling=o.optBoolean("kdeInstalling"),
            kdeStage=o.optString("kdeStage",state.value.kdeStage),
            kdeError=kdeError,
            message=message
        )
    }
    ...
    private suspend fun refresh() {
        runCatching {
            bridge?.let { applyStatus(bridgeString("status"){it.status()}) }
        }.onFailure {
            state.value=state.value.copy(message=it.message?:"Status unavailable")
        }
    }
    ...
    fun startVm()=operation { b ->
        ...
        state.value=state.value.copy(
            ...
            message=if(reconnectProbeArmed)
                "VM restarted. Run a guest command to verify reconnect."
            else
                "Managed Microdroid running"
        )
    }

    fun stopVm()=operation { b ->
        pendingSurface=null
        val wasMicrodroid=state.value.mode=="microdroid"
        applyStatus(bridgeString("vm_stop"){b.stopVm()})
        if(successfulCommands>0&&wasMicrodroid) reconnectProbeArmed=true
        state.value=state.value.copy(
            reconnect=if(reconnectProbeArmed)"PENDING" else state.value.reconnect,
            message="VM stopped; data retained"
        )
    }
    ...
    fun probeCapabilities()=operation { b ->
        ...
        val text=if(result.optBoolean("ok")) result.optString("text")
        else "BLOCKED: ${result.optString("error")}"
    ...
    fun startDebian(width:Int,height:Int,dpi:Int,refreshRate:Int)=operation { b ->
        ...
        state.value=state.value.copy(
            message=if(state.value.debianProtected)
                "Protected Debian VM is running; testing console and display"
            else
                "Debian VM is running; testing console and display"
        )

        pendingSurface?.let { surface ->
            if(surface.isValid) {
                runCatching {
                    withContext(Dispatchers.IO){b.setDisplaySurface(surface)}
                }.onFailure {
                    state.value=state.value.copy(message="Debian booted; display attach blocked: ${it.message}")
                }
            }
        }
        ...
        runCatching {
            JSONObject(
                bridgeString("debian_identity"){
                    b.debianConsole("cat /etc/os-release; uname -a; id")
                }
            )
        }.onSuccess { id ->
            if(id.optBoolean("ok")) {
                state.value=state.value.copy(
                    debianTerminal=(state.value.debianTerminal+
                        "\n# identity\n${id.optString("output")}").takeLast(200000),
                    message="Debian guest verified"
                )
            } else {
                state.value=state.value.copy(
                    message="Debian VM running; serial console not ready: ${id.optString("error")}"
                )
            }
        }.onFailure {
            state.value=state.value.copy(
                message="Debian VM running; identity probe failed: ${it.message}"
            )
        }
        refresh()
    }
    ...
    fun debianConsole(command:String)=operation { b ->
        ...
        state.value=state.value.copy(
            debianTerminal=(state.value.debianTerminal+
                "\n# $command\n${result.optString("output")}").takeLast(240000),
    ...
    fun installKde()=operation { b ->
        applyStatus(bridgeString("kde"){b.installKde()})
        state.value=state.value.copy(message="KDE Plasma provisioning started")
    }
    ...
    fun attachSurface(surface:Surface) {
        ...
        scope.launch {
            runCatching {
                ...
                state.value=state.value.copy(message="Display attach failed: ${it.message}")
            }
            refresh()
        }
    }
    ...
    fun detachSurface(surface:Surface?=null) {
        ...
        val b=bridge?:return
        scope.launch {
            runCatching { withContext(Dispatchers.IO){b.clearDisplaySurface()} }
            refresh()
        }
    }

    fun sendKey(action:Int,keyCode:Int,scanCode:Int,metaState:Int)=
        runCatching { bridge?.sendKey(action,keyCode,scanCode,metaState)?:false }.getOrDefault(false)

    fun sendTouch(action:Int,x:Float,y:Float,pointerId:Int)=
        runCatching { bridge?.sendTouch(action,x,y,pointerId)?:false }.getOrDefault(false)
