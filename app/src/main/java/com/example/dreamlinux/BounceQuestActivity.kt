...
class BounceQuestActivity : Activity() {
    ...
    private fun buildHud(root:FrameLayout){
        ...
        coinText=pill("✦  0 / 10",25f);root.addView(coinText,FrameLayout.LayoutParams(dp(190),dp(72),Gravity.TOP or Gravity.CENTER_HORIZONTAL).apply{topMargin=topPad})
        timerText=pill("◴  00:00.00",25f);root.addView(timerText,FrameLayout.LayoutParams(dp(235),dp(72),Gravity.TOP or Gravity.CENTER_HORIZONTAL).apply{topMargin=topPad;leftMargin=dp(400)})
        sfxText=pill("🔊  SFX ON",22f).apply{setOnClickListener{sfx=!sfx;text=if(sfx)"🔊  SFX ON" else "🔇  SFX OFF"}}
        root.addView(sfxText,FrameLayout.LayoutParams(dp(210),dp(70),Gravity.TOP or Gravity.RIGHT).apply{rightMargin=dp(115);topMargin=topPad})
    ...
    private fun consumeEvent(e:String){if(!sfx)return;when{e.startsWith("SFX:BOUNCE")->play(if(System.nanoTime() and 1L==0L)"bounce1" else "bounce2",.78f,.92f+Math.random().toFloat()*.14f);e=="SFX:JUMP"->play("jump",.86f,1f);e=="SFX:DASH"->play("dash",.82f,1f);e=="SFX:COLLECT"->play("collect",.88f,.98f+Math.random().toFloat()*.08f);e=="SFX:DEATH"->play("death",.92f,1f);e=="SFX:WIN"->play("win",1f,1f)}}
    private fun play(name:String,vol:Float,rate:Float){soundIds[name]?.let{sounds.play(it,vol,vol,2,0,rate.coerceIn(.5f,2f))}}

    private fun pill(text:String,size:Float)=label(text,size,true).apply{gravity=Gravity.CENTER;background=roundRect(0xCC25303B.toInt(),18f);setTextColor(Color.WHITE)}
    ...
    private fun roundRect(fill:Int,radius:Float)=GradientDrawable().apply{shape=GradientDrawable.RECTANGLE;color=android.content.res.ColorStateList.valueOf(fill);cornerRadius=dp(radius.toInt()).toFloat()}
    private fun circle(fill:Int,stroke:Int,strokeWidth:Int)=GradientDrawable().apply{shape=GradientDrawable.OVAL;color=android.content.res.ColorStateList.valueOf(fill);setStroke(dp(strokeWidth),stroke)}
