    ...
    private fun launchDaemon() {
        ...
        val command = """
            ...
            : > "${'$'}LOG"
            {
              ...
              # process whose command line actually ends in one of our daemon scripts.
              echo "[vessel-launch] stopping stale daemon"
              OLD_PID="${'$'}(pgrep -f '(^|/)python(3)? .*vessel_runtime_daemon(_v4)?\.py${'$'}' | head -n1 || true)"
              if [ -n "${'$'}OLD_PID" ]; then
              ...
              echo "[vessel-launch] exec runtime daemon protocol 4"
              exec python ~/vessel-poc-runtime/tools/venus_poc/vessel_runtime_daemon_v4.py
            } >> "${'$'}LOG" 2>&1
        """.trimIndent()
