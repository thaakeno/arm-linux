...
private:
    ...
    void handleClient(int c){
        while(!stop_){GpuHdr h{};int recvfd=recvHeaderWithFd(c,h);if(recvfd==-2)return;if(h.flags&GPU_REPLY){if(recvfd>=0)close(recvfd);return;}if(h.size>32*1024*1024){if(recvfd>=0)close(recvfd);return;}std::vector<uint8_t> body(h.size);if(h.size&&!recvAll(c,body.data(),body.size())){if(recvfd>=0)close(recvfd);return;}
            ...
            else if(h.request==GPU_CURSOR_UPDATE){if(body.size()>=sizeof(CursorUpdate)+4096*4){CursorUpdate u{};memcpy(&u,body.data(),sizeof(u));std::lock_guard<std::mutex>g(lock_);cursorX_=u.pos.x;cursorY_=u.pos.y;hotX_=u.hot_x;hotY_=u.hot_y;const uint8_t*px=body.data()+sizeof(u);for(size_t i=0;i<4096;i++){uint32_t v;memcpy(&v,px+i*4,4);cursorPixels_[i]=0xff000000u|(v&0x00ffffffu);}cursorVisible_=true;cursorSerial_++;}if(recvfd>=0)close(recvfd);}
