...
void boxGeom(int local,vec3 c,vec3 h,out vec3 p,out vec3 n,out vec2 uv){int f=local/6,co=local-f*6;vec2 q=tri(co)*2.0-1.0;p=c+boxPos(f,q)*h;n=boxN(f);uv=tri(co)*vec2(max(h.x,h.z)*.65,max(h.y,.2)*.65);}
...
vec3 coinPos(int i){
 ...
 if(i==4)return vec3(1.2,1.45,-7.3);if(i==5)return vec3(4.4,1.75,-9.1);if(i==6)return vec3(1.4,2,-11);if(i==7)return vec3(-3.9,2.75,-14.6);
 if(i==8)return vec3(2.6,3.45,-18.2);return vec3(0,4.25,-27);
}
...
mat4 viewProj(vec3 eye,vec3 target,float aspect){
 ...
 float zN=.08,zF=90.0,t=1.0/tan(radians(54.0)*.5);mat4 p=mat4(vec4(t/aspect,0,0,0),vec4(0,t,0,0),vec4(0,0,zF/(zN-zF),-1),vec4(0,0,(zN*zF)/(zN-zF),0));return p*v;
}
