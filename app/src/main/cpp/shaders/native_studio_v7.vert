...
const int QUALITY_SPHERES=5; const int QUALITY_VERTS=STATIC_VERTS+QUALITY_SPHERES*SPHERE_VERTS;
...
void emitHero(int local,out vec3 P,out vec3 N,out vec2 uv,out int M,out int O,out vec3 T,out vec3 B){
    ...
    vec2 tiled=fract(uv*1.28+vec2(face*.173,face*.071)); float h=textureLod(concreteHeight,tiled,0.0).r-.5;
    float edgeDist=1.0-max(abs(q.x),abs(q.y)),seam=smoothstep(0.0,.085,edgeDist),depth=mix(.028,.060,clamp(pc.quality/100.0,0.0,1.0));
