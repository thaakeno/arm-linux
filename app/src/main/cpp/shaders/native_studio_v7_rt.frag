...
// v7.8 lighting pass: sparse Adreno ray queries, one stochastic GI bounce, emissive wall torches and richer material response.
...
Material materialAt(int m,vec3 P,vec2 uv,out float water){
    ...
    else if(m==3){float pores=hash21(floor(uv*230.0)),scuff=smoothstep(.86,.99,hash21(floor(uv*35.0+9.0)));a.base=mix(vec3(.19,.004,.004),vec3(.74,.016,.010),pores*.38);a.base=mix(a.base,vec3(.10,.018,.014),scuff*.30);a.roughness=clamp(.60+pores*.12-scuff*.08,.52,.78);a.clearcoat=.11;a.ao=.95;}
    ...
    else if(m==7){float grain=hash21(floor(uv*220.0)),scuff=smoothstep(.88,.995,hash21(floor(uv*31.0+4.0)));a.base=mix(vec3(.018,.024,.030),vec3(.082,.108,.125),grain*.45);a.base*=1.0-scuff*.18;a.roughness=clamp(.64+grain*.12-scuff*.13,.50,.82);a.clearcoat=.055;a.ao=.94;}
    else if(m==8){float cell=hash21(floor(uv*100.0+P.xy*3.0)),vein=hash21(floor(uv*27.0+vec2(8.0,3.0)));a.base=mix(vec3(.012,.085,.025),vec3(.045,.34,.095),cell*.64);a.base*=.84+.18*vein;a.roughness=clamp(.26+.15*cell,.22,.46);a.clearcoat=.56;a.ao=.96;}
...
vec3 postProcess(vec3 hdr){vec3 c=aces(max(hdr,vec3(0)));float l=dot(c,vec3(.2126,.7152,.0722));c=mix(vec3(l),c,1.07);c=(c-.5)*1.04+.5;float hi=max(c.r,max(c.g,c.b)),bloom=smoothstep(.76,1.0,hi);c+=c*bloom*.032;return pow(clamp(c,0.0,1.0),vec3(1.0/2.2));}
...
void bodyHitInfo(int bi,vec3 hp,out vec3 hn,out vec2 huv){BodyGpu body=bodies[bi];vec3 v=hp-body.posRad.xyz,axis=body.extra.yzw;float dl=length(axis);axis=dl>.0001?axis/dl:vec3(0,1,0);if(body.meta.y==1){bool slime=body.meta.z==1;float maxC=slime?.50:(body.meta.x==3?.40:.30);float minA=slime?.48:(body.meta.x==3?.60:.70);float c=clamp(body.extra.x,-.025,maxC),along=clamp(1.0-c,minA,1.025),perp=inversesqrt(along);float va=dot(v,axis);vec3 vp=v-axis*va;float ar=body.posRad.w*along,pr=body.posRad.w*perp;hn=normalize(axis*(va/max(ar*ar,1e-5))+vp/max(pr*pr,1e-5));vec3 local=normalize(axis*(va/max(ar,1e-4))+vp/max(pr,1e-4));huv=vec2(atan(local.z,local.x)/(2.0*PI)+.5,acos(clamp(local.y,-1.0,1.0))/PI);}else{vec3 n=normalize(v);hn=n;huv=vec2(atan(n.z,n.x)/(2.0*PI)+.5,acos(clamp(n.y,-1.0,1.0))/PI);}}
...
vec3 pathIndirect(vec3 P,vec3 N,vec3 V,Material first,int source,float seed){
    ...
    vec3 sunL=normalize(vec3(-.53,.67,.52));vec3 lv=bodies[2].posRad.xyz-hp;float ld2=max(dot(lv,lv),.35);vec3 bounced=ibl(hit,hn,-rd)*.48+brdf(hit,hn,-rd,sunL,vec3(1.40,1.18,.98))*.34+brdf(hit,hn,-rd,normalize(lv),max(bodies[2].extra.yzw,vec3(.05))*(18.0/ld2))*.28+torchLighting(hit,hn,-rd,hp)*.55;
