...
float waterCoverage(vec2 uv){
    vec2 t=fract(uv);float h=texture(floorHeight,t).r,wet=clamp(pc.wetness,0.0,1.0),level=mix(.30,.88,wet);
    float basin=1.0-smoothstep(level-.10,level+.018,h),edge=min(min(t.x,t.y),min(1.0-t.x,1.0-t.y)),joints=1.0-smoothstep(.018,.085,edge);
    float patch=step(.74,hash21(floor(uv*5.0+vec2(7.0,13.0))))*wet*.28;
    return clamp(max(basin,max(joints*wet*.96,patch)),0.0,1.0);
}
Material materialAt(int m,vec3 P,vec2 uv,out float water){
    ...
    else if(m==1){vec2 t=fract(uv);vec4 arm=texture(floorArm,t);water=waterCoverage(t);vec3 stone=texture(floorAlbedo,t).rgb;float grain=.90+.14*hash21(floor(t*96.0));a.base=stone*grain;a.base=mix(a.base,a.base*vec3(.16,.21,.27),water*.94);a.roughness=mix(clamp(arm.g,.32,.90),mix(.060,.018,water),water);a.clearcoat=water;a.ao=mix(arm.r,1.0,water*.36);}
...
vec3 materialNormal(int m,vec3 P,vec2 uv,vec3 N,vec3 T,vec3 B,float water){
    ...
    if(m==1)return tangentNormal(floorNormal,fract(uv),N,T,B,mix(1.42,.24,water)*detail);
...
void main(){
    ...
    float NoV=max(dot(N,V),0.0);vec3 F=fresnelSchlick(NoV,mix(vec3(.04),m.base,m.metallic));bool reflective=(inMaterial==2)||(inMaterial==1&&water>.52)||(m.clearcoat>.70);float importance=max(F.r,max(F.g,F.b))*(1.0-m.roughness*.75)*budget,rayGate=hash21(floor(gl_FragCoord.xy*.5)),rayRate=mix(.025,.11,budget);
    if(pc.rtEnabled>.5&&reflective&&importance>.040&&rayGate<rayRate){vec3 R=reflect(-V,N),hp,hn;int hm;vec2 huv;int hid=traceScene(inWorldPos+geomN*rayEps(inObject),R,inObject,hp,hn,hm,huv);vec3 refl=envSpec(R,m.roughness);if(hid>=0){float hw;Material hit=materialAt(hm,hp,huv,hw);vec3 hitV=normalize(inWorldPos-hp);if(dot(hn,hitV)<0)hn=-hn;refl=hit.emission+hit.base*.24+ibl(hit,hn,hitV)*.46;}float strength=inMaterial==1?water*.62:(inMaterial==2?.84:(m.clearcoat>.70?.34:.52));color=mix(color,refl,clamp(F*strength,vec3(0),vec3(.88)));}
    if(inMaterial==1&&water>.01){vec3 R=reflect(-V,N);float f=.07+.48*pow(1.0-NoV,5.0);vec3 wetRefl=envSpec(R,mix(.014,.085,1.0-water));color=mix(color,wetRefl,clamp(water*(.38+f),0.0,.82));}
