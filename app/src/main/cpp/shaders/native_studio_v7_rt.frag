...
float fbm2(vec2 p){float f=0.0,a=.56;for(int i=0;i<3;i++){f+=a*hash21(floor(p));p=p*2.03+17.1;a*=.47;}return f;}
...
float waterCoverage(vec2 uv){
    vec2 t=fract(uv);float h=texture(floorHeight,t).r,wet=clamp(pc.wetness,0.0,1.0),level=mix(.27,.84,wet);
    float basin=1.0-smoothstep(level-.12,level+.020,h),edge=min(min(t.x,t.y),min(1.0-t.x,1.0-t.y)),joints=1.0-smoothstep(.020,.095,edge);
    float broad=smoothstep(.46,.76,fbm2(uv*4.2+vec2(4.7,11.3))),breakup=.78+.22*fbm2(uv*17.0);
    return clamp(max(basin*breakup,max(joints*wet*.92,broad*wet*.38)),0.0,1.0);
}
Material materialAt(int m,vec3 P,vec2 uv,out float water){
    ...
    else if(m==1){vec2 t=fract(uv);vec4 arm=texture(floorArm,t);water=waterCoverage(t);vec3 stone=texture(floorAlbedo,t).rgb;float soil=.80+.22*fbm2(t*19.0+P.xz*.15),mineral=.90+.14*fbm2(t*53.0);a.base=stone*soil*mineral;a.base=mix(a.base,a.base*vec3(.20,.24,.28),water*.93);a.roughness=mix(clamp(arm.g,.34,.92),mix(.075,.024,water),water);a.clearcoat=water;a.ao=mix(arm.r,1.0,water*.30);}
    else if(m==2){float grain=fbm2(uv*240.0),scratch=smoothstep(.965,.997,hash21(vec2(floor(uv.y*470.0),floor(uv.x*41.0)))),scuff=smoothstep(.84,.98,fbm2(uv*34.0+13.0));a.base=mix(vec3(.095,.105,.115),vec3(.34,.36,.39),grain*.34);a.base=mix(a.base,vec3(.48,.43,.35),scratch*.38);a.base*=1.0-scuff*.12;a.metallic=1.0;a.roughness=clamp(.17+grain*.20+scuff*.12,.15,.44);a.clearcoat=.035;}
    else if(m==3){float pores=fbm2(uv*220.0),scuff=smoothstep(.80,.97,fbm2(uv*31.0+9.0));a.base=mix(vec3(.20,.004,.004),vec3(.72,.015,.010),pores*.38);a.base=mix(a.base,vec3(.10,.018,.014),scuff*.30);a.roughness=clamp(.70+pores*.14-scuff*.09,.60,.89);a.ao=.95;}
    ...
    else if(m==5){vec2 t=fract(uv*1.12);vec4 arm=texture(concreteArm,t);float stain=.64+.36*fbm2(P.xz*2.2+uv*7.0);a.base=texture(concreteAlbedo,t).rgb*vec3(.46,.47,.47)*stain;a.roughness=clamp(arm.g+.10,.56,.98);a.ao=arm.r;}
    else if(m==6){vec2 t=fract(uv*1.20);vec4 arm=texture(darkArm,t);a.base=texture(darkAlbedo,t).rgb*vec3(.23,.24,.27);a.roughness=clamp(arm.g*.78,.20,.77);a.ao=arm.r;}
    else if(m==7){float grain=fbm2(uv*205.0),scuff=smoothstep(.84,.98,fbm2(uv*27.0+4.0)),dust=smoothstep(.78,.97,fbm2(uv*10.0+19.0));a.base=mix(vec3(.020,.026,.032),vec3(.090,.115,.130),grain*.45);a.base=mix(a.base,vec3(.18,.17,.15),dust*.10);a.base*=1.0-scuff*.20;a.roughness=clamp(.75+grain*.13-scuff*.16,.56,.93);a.ao=.94;}
    else if(m==8){float cell=fbm2(uv*92.0+P.xy*2.6),vein=fbm2(uv*23.0+vec2(8.0,3.0));a.base=mix(vec3(.016,.11,.032),vec3(.060,.40,.12),cell*.64);a.base*=.84+.18*vein;a.roughness=clamp(.32+.17*cell,.28,.54);a.clearcoat=.40;a.ao=.96;}
    else if(m==10){float n=hash21(floor(P.xz*15.0+P.yy*7.0));a.base=mix(vec3(.04,.12,.05),vec3(.11,.28,.09),n);a.roughness=.74;a.ao=.84;}
    else if(m==12){float stain=.66+.34*fbm2(P.xz*5.0+P.yy);a.base=vec3(.050,.057,.064)*stain;a.roughness=.86;a.metallic=.02;}
...
vec3 materialNormal(int m,vec3 P,vec2 uv,vec3 N,vec3 T,vec3 B,float water){
    ...
    if(m==1){vec3 n=tangentNormal(floorNormal,fract(uv),N,T,B,mix(1.25,.30,water)*detail);if(water>.20){float r1=fbm2(P.xz*18.0+pc.time*.035)-.5,r2=fbm2(P.zx*21.0-pc.time*.028)-.5;n=normalize(n+(T*r1+B*r2)*water*.025);}return n;}
    if(m==6)return tangentNormal(darkNormal,fract(uv*1.20),N,T,B,.84*detail);
    if(m==2||m==3||m==7||m==8){float n1=fbm2(uv*190.0)-.5,n2=fbm2(uv.yx*233.0+11.0)-.5;float amp=m==2?.030:(m==8?.014:(m==3?.020:.025));return normalize(N+(T*n1+B*n2)*amp*detail);}return normalize(N);
}
...
vec3 ibl(Material m,vec3 N,vec3 V){vec3 R=reflect(-V,N),F0=mix(vec3(.04),m.base,m.metallic),F=fresnelSchlick(max(dot(N,V),0.0),F0);vec3 diff=skySample(N)*m.base*(1.0-m.metallic)*(.27*m.ao),spec=envSpec(R,m.roughness)*F;if(m.clearcoat>0)spec+=envSpec(R,max(.025,m.roughness*.55))*fresnelSchlick(max(dot(N,V),0.0),vec3(.04))*m.clearcoat*.54;return diff+spec;}
vec3 aces(vec3 x){x*=exp2(pc.exposure);const float a=2.51,b=.03,c=2.43,d=.59,e=.14;return clamp((x*(a*x+b))/(x*(c*x+d)+e),0.0,1.0);}
vec3 postProcess(vec3 hdr){vec3 c=aces(max(hdr,vec3(0)));float l=dot(c,vec3(.2126,.7152,.0722));c=mix(vec3(l),c,1.09);c=(c-.5)*1.055+.5;float hi=max(c.r,max(c.g,c.b)),bloom=smoothstep(.76,1.0,hi);c+=c*bloom*.035;return pow(clamp(c,0.0,1.0),vec3(1.0/2.2));}

float rayEps(int obj){if(obj>=STATIC_INSTANCES){int bi=obj-STATIC_INSTANCES;return max(.0060,bodies[bi].posRad.w*.011);}return .0060;}
float shadowVisibility(vec3 P,vec3 N,vec3 target,int source){vec3 d=target-P;float L=length(d);if(L<.05)return 1.0;d/=L;float e=rayEps(source);rayQueryEXT rq;rayQueryInitializeEXT(rq,topLevelAS,gl_RayFlagsTerminateOnFirstHitEXT|gl_RayFlagsOpaqueEXT,0x01,P+N*e,e,d,max(.02,L-.060));rayQueryProceedEXT(rq);if(rayQueryGetIntersectionTypeEXT(rq,true)==gl_RayQueryCommittedIntersectionNoneEXT)return 1.0;int id=int(rayQueryGetIntersectionInstanceCustomIndexEXT(rq,true));return id==source?1.0:.045;}
...
vec3 floorHitNormal(vec2 uv){vec2 e=vec2(1.0/2048.0);float hx=texture(floorHeight,fract(uv+vec2(e.x,0))).r-texture(floorHeight,fract(uv-vec2(e.x,0))).r;float hz=texture(floorHeight,fract(uv+vec2(0,e.y))).r-texture(floorHeight,fract(uv-vec2(0,e.y))).r;return normalize(vec3(-hx*4.0,1,-hz*4.0));}
void bodyHitInfo(int bi,vec3 hp,out vec3 hn,out vec2 huv){BodyGpu body=bodies[bi];vec3 v=hp-body.posRad.xyz,axis=body.extra.yzw;float dl=length(axis);axis=dl>.0001?axis/dl:vec3(0,1,0);if(body.meta.y==1){bool slime=body.meta.z==1;float c=clamp(body.extra.x,-.045,slime?.46:.32),along=clamp(1.0-c,slime?.50:.64,1.05),perp=inversesqrt(along);float va=dot(v,axis);vec3 vp=v-axis*va;float ar=body.posRad.w*along,pr=body.posRad.w*perp;hn=normalize(axis*(va/max(ar*ar,1e-5))+vp/max(pr*pr,1e-5));vec3 local=normalize(axis*(va/max(ar,1e-4))+vp/max(pr,1e-4));huv=vec2(atan(local.z,local.x)/(2.0*PI)+.5,acos(clamp(local.y,-1.0,1.0))/PI);}else{vec3 n=normalize(v);hn=n;huv=vec2(atan(n.z,n.x)/(2.0*PI)+.5,acos(clamp(n.y,-1.0,1.0))/PI);}}
int traceScene(vec3 origin,vec3 dir,int source,out vec3 hp,out vec3 hn,out int hm,out vec2 huv){rayQueryEXT rq;float e=source>=0?rayEps(source):.006;rayQueryInitializeEXT(rq,topLevelAS,gl_RayFlagsOpaqueEXT,0x03,origin,e,dir,42.0);while(rayQueryProceedEXT(rq)){}if(rayQueryGetIntersectionTypeEXT(rq,true)==gl_RayQueryCommittedIntersectionNoneEXT)return -1;int id=int(rayQueryGetIntersectionInstanceCustomIndexEXT(rq,true));float t=rayQueryGetIntersectionTEXT(rq,true);hp=origin+dir*t;if(id==0){hm=1;huv=(hp.xz/14.1+.5)*3.20;hn=floorHitNormal(huv);}else if(id==1){vec3 he=vec3(1.22,1.06,1.22);hn=boxNormal(hp,vec3(0),he);hm=mod(pc.heroMaterial,10.0)<.5?0:6;huv=planarUv(hp,hn);}else if(id<STATIC_INSTANCES){vec3 c,s;boxInfo(id,c,s,hm);hn=boxNormal(hp,c,s);huv=planarUv(hp,hn);}else{int bi=id-STATIC_INSTANCES;bodyHitInfo(bi,hp,hn,huv);hm=bodies[bi].meta.x;}return id;}
...
void main(){
    ...
    float water;Material m=materialAt(inMaterial,inWorldPos,inUv,water);vec3 V=normalize(inCameraPos-inWorldPos),T=normalize(inTangent),B=normalize(inBitangent),geomN=normalize(inWorldNormal);if(dot(geomN,V)<0)geomN=-geomN;vec3 N=materialNormal(inMaterial,inWorldPos,inUv,geomN,T,B,water);if(dot(N,V)<0)N=-N;
    ...
    if(pt){float localVis=shadowVisibility(inWorldPos,N,bodies[2].posRad.xyz,inObject);vec3 direct=ibl(m,N,V)*.48+brdf(m,N,V,sunL,vec3(1.55,1.30,1.08)*(1.0-night*.80));direct+=brdf(m,N,V,localL,lightRgb*(25.0/ld2))*localVis;float seed=dot(floor(gl_FragCoord.xy),vec2(12.9898,78.233))+float(inObject)*19.17;vec3 indirect=pathIndirect(inWorldPos,N,V,m,inObject,seed);outColor=vec4(postProcess(direct+indirect*.72),1);return;}
    float budget=clamp(pc.rtBudget,0.0,1.0),sunScore=max(dot(N,sunL),0.0)*1.9*(1.0-night*.82),localScore=max(dot(N,localL),0.0)*(25.0/ld2),vis=1.0;bool important=inObject==0||inObject==1||(inObject>=STATIC_INSTANCES&&inObject<STATIC_INSTANCES+5);if(pc.rtEnabled>.5&&budget>.05&&important&&(sunScore>.015||localScore>.015)){vec3 target=localScore>sunScore?bodies[2].posRad.xyz:inWorldPos+sunL*36.0;vis=shadowVisibility(inWorldPos,N,target,inObject);}vec3 color=ibl(m,N,V);color+=brdf(m,N,V,sunL,vec3(2.0,1.62,1.24)*(1.0-night*.82))*((sunScore>=localScore)?vis:1.0);color+=brdf(m,N,V,localL,lightRgb*(25.0/ld2))*((localScore>sunScore)?vis:1.0);
    float NoV=max(dot(N,V),0.0);vec3 F=fresnelSchlick(NoV,mix(vec3(.04),m.base,m.metallic));bool reflective=(inMaterial==2)||(inMaterial==1&&water>.46)||(m.clearcoat>.62);float importance=max(F.r,max(F.g,F.b))*(1.0-m.roughness*.75)*budget,rayGate=hash21(floor(gl_FragCoord.xy*.5)),rayRate=mix(.035,.16,budget);
    if(pc.rtEnabled>.5&&reflective&&importance>.035&&rayGate<rayRate){vec3 R=reflect(-V,N),hp,hn;int hm;vec2 huv;int hid=traceScene(inWorldPos+N*rayEps(inObject),R,inObject,hp,hn,hm,huv);vec3 refl=envSpec(R,m.roughness);if(hid>=0){float hw;Material hit=materialAt(hm,hp,huv,hw);vec3 hitV=normalize(inWorldPos-hp);if(dot(hn,hitV)<0)hn=-hn;refl=hit.emission+hit.base*.24+ibl(hit,hn,hitV)*.46;}float strength=inMaterial==1?water*.62:(inMaterial==2?.84:(m.clearcoat>.62?.34:.52));color=mix(color,refl,clamp(F*strength,vec3(0),vec3(.88)));}
    if(inMaterial==1&&water>.01){vec3 R=reflect(-V,N);float f=.07+.48*pow(1.0-NoV,5.0);vec3 wetRefl=envSpec(R,mix(.018,.10,1.0-water));color=mix(color,wetRefl,clamp(water*(.34+f),0.0,.78));}
