...
// v7.7: Adreno-first HWRT. Hybrid stays sparse; path inspection uses one stochastic indirect bounce.
...
void bodyHitInfo(int bi,vec3 hp,out vec3 hn,out vec2 huv){BodyGpu body=bodies[bi];vec3 v=hp-body.posRad.xyz,axis=body.extra.yzw;float dl=length(axis);axis=dl>.0001?axis/dl:vec3(0,1,0);if(body.meta.y==1){bool slime=body.meta.z==1;float c=clamp(body.extra.x,-.020,slime?.32:.30),along=clamp(1.0-c,slime?.64:.68,1.020),perp=inversesqrt(along);float va=dot(v,axis);vec3 vp=v-axis*va;float ar=body.posRad.w*along,pr=body.posRad.w*perp;hn=normalize(axis*(va/max(ar*ar,1e-5))+vp/max(pr*pr,1e-5));vec3 local=normalize(axis*(va/max(ar,1e-4))+vp/max(pr,1e-4));huv=vec2(atan(local.z,local.x)/(2.0*PI)+.5,acos(clamp(local.y,-1.0,1.0))/PI);}else{vec3 n=normalize(v);hn=n;huv=vec2(atan(n.z,n.x)/(2.0*PI)+.5,acos(clamp(n.y,-1.0,1.0))/PI);}}
...
vec3 sampleBounce(Material m,vec3 N,vec3 V,float a,float b){vec3 F0=mix(vec3(.04),m.base,m.metallic);float specChance=clamp(max(F0.r,max(F0.g,F0.b))*(1.0-m.roughness*.45)+m.metallic*.55,.08,.94);if(a<specChance){vec3 R=reflect(-V,N),j=cosineHemisphere(R,vec2(fract(a*7.31+.17),b));return normalize(mix(R,j,m.roughness*m.roughness*.58));}return cosineHemisphere(N,vec2(fract(a*5.17+.31),b));}
vec3 pathIndirect(vec3 P,vec3 N,vec3 V,Material first,int source,float seed){
    ...
    vec3 F0=mix(vec3(.04),first.base,first.metallic),F=fresnelSchlick(max(dot(N,V),0.0),F0);vec3 throughput=mix(first.base*(1.0-first.metallic),F,clamp(first.metallic+.22*(1.0-first.roughness),0.0,1.0))*mix(.56,.88,1.0-first.roughness);
    if(hid<0)return min(throughput*skySample(rd),vec3(7.0));float w;Material hit=materialAt(hm,hp,huv,w);hn=normalize(hn);if(dot(hn,-rd)<0)hn=-hn;applyHeroWet(hit,hid,hm,hp,huv,hn);
    if(length(hit.emission)>0){vec3 e=hit.emission;if(hm==4&&hid==STATIC_INSTANCES+2)e=max(bodies[2].extra.yzw,vec3(.05))*8.6;return min(throughput*e,vec3(7.0));}
    vec3 sunL=normalize(vec3(-.53,.67,.52));vec3 lv=bodies[2].posRad.xyz-hp;float ld2=max(dot(lv,lv),.35);vec3 smoothBounce=ibl(hit,hn,-rd)*.62+brdf(hit,hn,-rd,sunL,vec3(1.35,1.15,.98))*.30+brdf(hit,hn,-rd,normalize(lv),max(bodies[2].extra.yzw,vec3(.05))*(18.0/ld2))*.24;
    return min(throughput*smoothBounce,vec3(7.0));
}
...
void main(){
    ...
    if(pt){float localVis=shadowVisibility(inWorldPos,geomN,bodies[2].posRad.xyz,inObject);vec3 direct=ibl(m,N,V)*.58+brdf(m,N,V,sunL,vec3(1.55,1.30,1.08)*(1.0-night*.80));direct+=brdf(m,N,V,localL,lightRgb*(25.0/ld2))*localVis;float seed=dot(floor(gl_FragCoord.xy),vec2(12.9898,78.233))+float(inObject)*19.17;vec3 indirect=pathIndirect(inWorldPos,geomN,V,m,inObject,seed);vec3 stableGI=mix(ibl(m,N,V),indirect,.46);outColor=vec4(postProcess(direct+stableGI*.58),1);return;}
    ...
    float NoV=max(dot(N,V),0.0);vec3 F=fresnelSchlick(NoV,mix(vec3(.04),m.base,m.metallic));bool heroWet=inObject==1&&m.clearcoat>.08;bool reflective=(inMaterial==2)||(inMaterial==1&&water>.44)||heroWet||(m.clearcoat>.70);float importance=max(F.r,max(F.g,F.b))*(1.0-m.roughness*.75)*budget,rayGate=hash21(floor(gl_FragCoord.xy*.5)),rayRate=mix(.032,.15,budget);
    if(pc.rtEnabled>.5&&reflective&&importance>.034&&rayGate<rayRate){vec3 R=reflect(-V,N),hp,hn;int hm;vec2 huv;int hid=traceScene(inWorldPos+geomN*rayEps(inObject),R,inObject,hp,hn,hm,huv);vec3 refl=envSpec(R,m.roughness);if(hid>=0){float hw;Material hit=materialAt(hm,hp,huv,hw);vec3 hitV=normalize(inWorldPos-hp);if(dot(hn,hitV)<0)hn=-hn;applyHeroWet(hit,hid,hm,hp,huv,hn);refl=hit.emission+hit.base*.22+ibl(hit,hn,hitV)*.50;}float strength=inMaterial==1?water*.46:(inMaterial==2?.88:(heroWet?.30:(m.clearcoat>.70?.36:.52)));color=mix(color,refl,clamp(F*strength,vec3(0),vec3(.80)));}
