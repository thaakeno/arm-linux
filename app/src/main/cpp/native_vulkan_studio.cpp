...
constexpr const char* kLogTag = "VesselVulkan";
constexpr uint32_t kQualityVertexCount = 36u + 6u + (32u * 16u * 6u) * 3u;
constexpr uint32_t kStressVertexCount = kQualityVertexCount + (16u * 8u * 6u) * 64u;
...
class VulkanStudioRenderer {
public:
    explicit VulkanStudioRenderer(ANativeWindow* window) : window_(window) {
        ...
        appendLog("surface attached");
    }

    ~VulkanStudioRenderer() {
        stop();
        if (window_) {
    ...
    void rotate(float yawDegrees, float pitchDegrees) {
        yaw_.store(yaw_.load(std::memory_order_relaxed) + yawDegrees * 0.01745329252f,
                   std::memory_order_relaxed);
        float nextPitch = pitch_.load(std::memory_order_relaxed) + pitchDegrees * 0.01745329252f;
        pitch_.store(std::clamp(nextPitch, -1.28f, 1.28f), std::memory_order_relaxed);
    }

    void zoom(float scaleFactor) {
        if (!(scaleFactor > 0.01f)) return;
        const float next = cameraDistance_.load(std::memory_order_relaxed) / scaleFactor;
        cameraDistance_.store(std::clamp(next, 3.8f, 13.0f), std::memory_order_relaxed);
    }

    void resize(int width, int height) {
        width = std::max(width, 1);
        height = std::max(height, 1);
        const int oldW = visibleWidth_.exchange(width, std::memory_order_relaxed);
        const int oldH = visibleHeight_.exchange(height, std::memory_order_relaxed);
        if (oldW != width || oldH != height) {
            appendLog("view resize " + std::to_string(width) + "x" + std::to_string(height));
            resizeRequested_.store(true, std::memory_order_relaxed);
        }
    }

    void setStress(bool enabled) {
        const bool old = stressMode_.exchange(enabled, std::memory_order_relaxed);
        if (old != enabled) appendLog(enabled ? "mode -> STRESS" : "mode -> QUALITY");
    }
...
private:
    void appendLog(const std::string& line) {
        ...
        if (logLines_.size() > 9) logLines_.erase(logLines_.begin());
    }
    ...
    void initVulkan() {
        ...
        appInfo.pApplicationName = "Vessel Vulkan Studio";
        ...
        appendLog("Android Vulkan surface ready");
        ...
        if (physicalDevice_ == VK_NULL_HANDLE) throw std::runtime_error("No Vulkan graphics+present queue");
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice_, &familyCount, qprops.data());
        timestampsSupported_ = queueFamily_ < qprops.size() && qprops[queueFamily_].timestampValidBits > 0;
        appendLog(timestampsSupported_ ? "GPU timestamps enabled" : "GPU timestamps unavailable");
        ...
        if (timestampsSupported_) {
            ...
            checkVk(vkCreateQueryPool(device_, &queryInfo, nullptr, &queryPool_), "vkCreateQueryPool");
        }
    ...
    void createDepthResources() {
        ...
        checkVk(vkAllocateMemory(device_, &allocInfo, nullptr, &depthMemory_), "vkAllocateMemory(depth)");
    ...
    void recreateSwapchain() {
        ...
        checkVk(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice_, surface_, &caps),
                "vkGetPhysicalDeviceSurfaceCapabilitiesKHR");

        uint32_t formatCount = 0;
        checkVk(vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice_, surface_, &formatCount, nullptr),
                "vkGetPhysicalDeviceSurfaceFormatsKHR(count)");
        if (formatCount == 0) throw std::runtime_error("Android Vulkan surface has no formats");
        std::vector<VkSurfaceFormatKHR> formats(formatCount);
        checkVk(vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice_, surface_, &formatCount, formats.data()),
        ...
        if (caps.currentExtent.width != UINT32_MAX) {
            ...
            swapchainExtent_.width = static_cast<uint32_t>(std::max(1, ANativeWindow_getWidth(window_)));
            swapchainExtent_.height = static_cast<uint32_t>(std::max(1, ANativeWindow_getHeight(window_)));
        }
        if (isQuarterTurn(preTransform_)) std::swap(swapchainExtent_.width, swapchainExtent_.height);
        ...
        for (uint32_t i = 0; i < actualCount; ++i) {
            ...
            viewInfo.subresourceRange.levelCount = 1;
            viewInfo.subresourceRange.layerCount = 1;
            checkVk(vkCreateImageView(device_, &viewInfo, nullptr, &imageViews_[i]), "vkCreateImageView(color)");
        }
        ...
        appendLog("swapchain " + std::to_string(swapchainExtent_.width) + "x" +
                  std::to_string(swapchainExtent_.height) + " " + rotationName(preTransform_));
    }
    ...
    void createRenderPassAndPipeline() {
        ...
        appendLog("PBR studio pipeline ready");
    }
    ...
    void recordCommandBuffer(uint32_t imageIndex) {
        ...
        clears[0].color = {{0.006f, 0.008f, 0.011f, 1.0f}};
        ...
        const float visibleW = static_cast<float>(std::max(1, visibleWidth_.load(std::memory_order_relaxed)));
        const float visibleH = static_cast<float>(std::max(1, visibleHeight_.load(std::memory_order_relaxed)));
        ...
        push.stressMode = stressMode_.load(std::memory_order_relaxed) ? 1.0f : 0.0f;
        vkCmdPushConstants(commandBuffer_, pipelineLayout_, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(push), &push);
        const uint32_t vertexCount = stressMode_.load(std::memory_order_relaxed) ? kStressVertexCount : kQualityVertexCount;
    ...
    void sampleGpuTime() {
        if (queryPool_ == VK_NULL_HANDLE || !queryIssued_) return;
        std::array<uint64_t, 2> ts{};
        const VkResult result = vkGetQueryPoolResults(
            device_, queryPool_, 0, 2, sizeof(ts), ts.data(), sizeof(uint64_t), VK_QUERY_RESULT_64_BIT);
        if (result == VK_SUCCESS && ts[1] >= ts[0]) {
            const double ns = static_cast<double>(ts[1] - ts[0]) * static_cast<double>(timestampPeriodNs_);
            gpuMs_.store(static_cast<float>(ns / 1000000.0), std::memory_order_relaxed);
        }
    }
    ...
    void drawFrame() {
        ...
        queryIssued_ = queryPool_ != VK_NULL_HANDLE;
    ...
    void renderLoop() {
        try {
            setStatus("Vulkan Studio · initializing…");
            initVulkan();
            appendLog("scene: cube + metal/ceramic/rubber spheres");
            appendLog("materials: procedural PBR + studio IBL approximation");
            ...
            while (running_.load(std::memory_order_relaxed)) {
                ...
                if (elapsed >= 0.5f) {
                    ...
                    const bool stress = stressMode_.load(std::memory_order_relaxed);
                    std::ostringstream s;
                    s.setf(std::ios::fixed);
                    s.precision(1);
                    s << "Vulkan Studio · " << gpuName_ << " · " << fps << " FPS";
                    s.precision(2);
                    s << " · GPU " << gpuMs_.load(std::memory_order_relaxed) << " ms";
                    s << " · " << (stress ? "STRESS" : "QUALITY") << "\n";
                    s << "view " << w << "x" << h << " · buffer "
            ...
            setStatus(std::string("ERROR: Vulkan Studio: ") + e.what());
        } catch (...) {
            appendLog("ERROR: unknown native exception");
            setStatus("ERROR: Vulkan Studio: unknown native exception");
        }
        cleanupVulkan();
    }
    ...
    void cleanupVulkan() {
        ...
        destroySwapchain();
        if (device_ != VK_NULL_HANDLE && queryPool_) vkDestroyQueryPool(device_, queryPool_, nullptr);
    ...
    std::string status_ = "Vulkan Studio · waiting for Surface…";
    mutable std::mutex logMutex_;
    std::vector<std::string> logLines_;

    std::atomic<float> yaw_{-0.48f};
    std::atomic<float> pitch_{0.34f};
    std::atomic<float> cameraDistance_{7.2f};
    std::atomic<bool> stressMode_{false};
...
VulkanStudioRenderer* fromHandle(jlong handle) {
    return reinterpret_cast<VulkanStudioRenderer*>(static_cast<intptr_t>(handle));
}
...
Java_com_example_dreamlinux_NativeCubeActivity_nativeCreate(JNIEnv* env, jobject, jobject surface) {
    ...
    if (!window) return 0;
    try {
        auto* renderer = new VulkanStudioRenderer(window);
        renderer->start();
...
extern "C" JNIEXPORT void JNICALL
Java_com_example_dreamlinux_NativeCubeActivity_nativeResize(JNIEnv*, jobject, jlong handle, jint width, jint height) {
    if (auto* renderer = fromHandle(handle)) renderer->resize(width, height);
}

extern "C" JNIEXPORT void JNICALL
Java_com_example_dreamlinux_NativeCubeActivity_nativeRotate(JNIEnv*, jobject, jlong handle, jfloat yawDegrees, jfloat pitchDegrees) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_example_dreamlinux_NativeCubeActivity_nativeSetStress(JNIEnv*, jobject, jlong handle, jboolean enabled) {
    if (auto* renderer = fromHandle(handle)) renderer->setStress(enabled == JNI_TRUE);
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_dreamlinux_NativeCubeActivity_nativeStatus(JNIEnv* env, jobject, jlong handle) {
    const std::string text = fromHandle(handle) ? fromHandle(handle)->status() : "Vulkan Studio · renderer not started";
    return env->NewStringUTF(text.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_dreamlinux_NativeCubeActivity_nativeLogs(JNIEnv* env, jobject, jlong handle) {
    const std::string text = fromHandle(handle) ? fromHandle(handle)->logs() : "renderer not started";
    return env->NewStringUTF(text.c_str());
}
