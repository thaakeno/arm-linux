...
def release_timestamp(releases):
    ...
    for rel in releases if isinstance(releases, list) else []:
        ...
        for key in ("unix-timestamp", "timestamp"):
            try: newest = max(newest, int(rel.get(key, 0) or 0))
...
def source_mtime():
    paths = []
    for pattern in ("/var/lib/app-info/yaml/*", "/var/cache/swcatalog/yaml/*",
                    "/var/lib/swcatalog/yaml/*", "/var/cache/swcatalog/xml/*",
...
def read_yaml(out):
    try:
        ...
        return
    paths = []
    for pattern in ("/var/lib/app-info/yaml/*.yml*", "/var/cache/swcatalog/yaml/*.yml*", "/var/lib/swcatalog/yaml/*.yml*"):
        paths.extend(glob.glob(pattern))
        ...
        try:
            with open_text(path) as fh:
                for raw in yaml.safe_load_all(fh):
                    ...
                    if isinstance(cats, str): cats = [x for x in re.split(r"[;,]", cats) if x]
                    icon = scalar(raw.get("Icon") or raw.get("icon"))
                    add(out, pkg, scalar(raw.get("Name") or raw.get("name")),
                        scalar(raw.get("Summary") or raw.get("summary")), friendly_category(cats),
                        scalar(raw.get("ID") or raw.get("id")), icon,
...
def read_xml(out):
    paths = []
    for pattern in ("/var/cache/swcatalog/xml/*.xml*", "/var/lib/app-info/xmls/*.xml*"):
        paths.extend(glob.glob(pattern))
    for path in sorted(set(paths)):
        try:
            with open_text(path) as fh:
                for event, comp in ET.iterparse(fh, events=("end",)):
...
def build_cache():
    apps = {}
    # XML iterparse is cheap on Bookworm; YAML fills gaps on images that ship it.
    read_xml(apps)
    read_yaml(apps)
    for pkg, name, summary, category in POPULAR:
        add(apps, pkg, name, summary, category)
    os.makedirs(os.path.dirname(CACHE_PATH), exist_ok=True)
    payload = {"version": CACHE_VERSION, "source_mtime": source_mtime(), "apps": list(apps.values())}
    fd, tmp = tempfile.mkstemp(prefix="app-catalog-v3-", suffix=".json", dir=os.path.dirname(CACHE_PATH))
    try:
...
def load_cache():
    try:
        with open(CACHE_PATH, "r", encoding="utf-8") as fh: payload = json.load(fh)
        if payload.get("version") != CACHE_VERSION or payload.get("source_mtime") != source_mtime(): raise ValueError("stale")
        return payload
    except Exception:
        return build_cache()
...
def installed_packages():
    out = {}
    try: text = open("/var/lib/dpkg/status", "r", encoding="utf-8", errors="replace").read()
    ...
    for block in text.split("\n\n"):
        ...
        if PACKAGE_RE.fullmatch(pkg) and status == "install ok installed": out[pkg] = size
    return out
...
def supplement_search(apps, query):
    ...
    if not found: return
    meta = run(["apt-cache", "show", "--no-all-versions", *found], 20)
    details = {}
    for block in meta.split("\n\n"):
        pkg = desc = section = ""; size = 0
        for line in block.splitlines():
            if line.startswith("Package: "): pkg = line[9:].strip()
        ...
        if PACKAGE_RE.fullmatch(pkg): details[pkg] = (desc, section, size)
    for pkg in found:
        if pkg in apps: continue
...
def b64_text(value): return base64.b64encode((value or "").encode("utf-8")).decode("ascii")
def b64_file(path):
    if not path: return ""
    try: return base64.b64encode(open(path, "rb").read()).decode("ascii")
...
def main():
    ...
    for item in by_pkg.values():
        ...
        if item["installed"] and installed[pkg] > 0: item["size"] = installed[pkg]
        if q and q not in (pkg + " " + item.get("name", "") + " " + item.get("summary", "") + " " + item.get("id", "")).lower(): continue
        if category != "All" and item.get("category", "Other") != category: continue
        apps.append(item)
