#!/usr/bin/env python3
"""Protocol-27 Vessel runtime.

Protocol 27 keeps the reconnectable guest command socket, fixes its inherited
10-second bootstrap timeout, and requires the VFRM2 tiled native desktop path.
No runtime monkey patching is used.
"""
...
class NativeRuntimeV27(v25.NativeRuntimeV25):
    ...
    def _ensure_agent(self) -> None:
        ...
        try:
            ...
            self.append("\n[vessel-agent] protocol-27 socket command channel ready\n")
        except Exception:
            self._close_agent()
            raise
        finally:
    ...
    def guest(self, command: str, timeout: float = 45.0) -> str:
        ...
        if self.proc is None or self.proc.poll() is not None: raise RuntimeError("Debian UML is not running")
        with self.command_lock:
            if not self.guest_ready: raise RuntimeError("Debian is not ready")
            self._ensure_agent()
            sock = self.agent_socket
            if sock is None: raise RuntimeError("guest command socket is unavailable")
            payload = command.encode("utf-8")
            if len(payload) > MAX_COMMAND: raise RuntimeError("guest command is too large")
            ...
            try:
                ...
                while True:
                    ...
                    if size > MAX_RECORD: raise RuntimeError(f"guest agent record too large: {size}")
                    ...
                    if kind == b"O":
                        ...
                        continue
                    if kind == b"D":
                        if len(body) != 4: raise RuntimeError("malformed guest completion record")
                        rc = struct.unpack("!i", body)[0]
                        text = output.decode("utf-8", "replace")
                        if rc != 0: raise RuntimeError(f"guest command failed rc={rc}: {text[-6000:]}")
                        return text
                    if kind == b"E": raise RuntimeError("guest command agent error: " + body.decode("utf-8", "replace"))
...
runtime = NativeRuntimeV27(input_bridge, frame_bridge)
...
def serve() -> None:
    ...
    print(f"[vessel-daemon] protocol={PROTOCOL_VERSION} command=socket-v27 display=VFRM2 input=native", flush=True)
    while True:
        conn, _ = srv.accept()
