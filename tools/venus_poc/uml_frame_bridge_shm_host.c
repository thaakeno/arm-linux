...
int main(int argc, char **argv) {
    ...
    if (xcb_connection_has_error(xc)) { fprintf(stderr, "[frame-host] X11 connect failed\n"); return 2; }
    ...
    xcb_create_window(xc, scr->root_depth, win, scr->root, 0, 0, 640, 480, 0,
                      XCB_WINDOW_CLASS_INPUT_OUTPUT, scr->root_visual,
                      XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK, values);
    xcb_change_property(xc, XCB_PROP_MODE_REPLACE, win, XCB_ATOM_WM_NAME,
                        XCB_ATOM_STRING, 8, 27, "UML Vulkan Shared Frame Bridge");
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (magic != MAGIC || !width || !height || stride < width*4u || size != stride*height || size > FRAME_SIZE) {
                fprintf(stderr, "[frame-host] invalid header magic=%08x %ux%u stride=%u size=%u\n", magic,width,height,stride,size); break;
            }
            if (frame < 5) { fprintf(stderr, "[frame-host] notification %ux%u stride=%u size=%u\n", width,height,stride,size); fflush(stderr); }
            uint32_t cfg[2] = { width, height };
            xcb_configure_window(xc, win, XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT, cfg);
            if (present_frame(xc, win, gc, scr->root_depth, width, height, stride, shared) < 0) break;
            frame++;
            if (frame <= 5 || frame % 60 == 0) { fprintf(stderr, "[frame-host] frame=%llu\n", frame); fflush(stderr); }
        }
        close(fd);
