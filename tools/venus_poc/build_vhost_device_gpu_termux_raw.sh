...
MARKER="$INSTALL/.vessel-raw-scanout-v2"
...
grep -Fq 'VESSEL_RAW_SCANOUT_V2' "$SRC/vhost-device-gpu/src/backend/virgl.rs"
grep -Fq 'stride = width.checked_mul(4)' "$SRC/vhost-device-gpu/src/backend/virgl.rs"
grep -Fq 'has_visible_rgb' "$SRC/vhost-device-gpu/src/backend/virgl.rs"
...
rm -f "$INSTALL/.vessel-raw-scanout-v1"
printf 'raw-scanout-v2\n' >"$MARKER"

echo "[vhost-gpu-raw] READY v2"
echo "[vhost-gpu-raw] rendering: guest GL -> VirGL -> ANGLE/Vulkan -> Adreno"
echo "[vhost-gpu-raw] readback: explicit width*4 stride, black frames suppressed"
