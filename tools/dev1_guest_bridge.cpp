...
void worker_accept_loop() {
    ...
    if (server < 0) { perror("worker vsock listen"); return; }
    fprintf(stderr, "DEV1 bridge worker vsock listening on %u\n", WORKER_PORT);
    for (;;) {
...
void http_loop() {
    ...
    if (server < 0) { perror("http listen"); return; }
    fprintf(stderr, "DEV1 HTTP proxy listening on 127.0.0.1:%d\n", HTTP_PORT);
    for (;;) {
        int c = ::accept4(server, nullptr, nullptr, SOCK_CLOEXEC);
        if (c < 0) { if (errno == EINTR) continue; break; }
...
void vnc_loop() {
    ...
    if (server < 0) { perror("vnc vsock listen"); return; }
    fprintf(stderr, "DEV1 VNC vsock listening on %u\n", VNC_PORT);
    for (;;) {
        int c = ::accept4(server, nullptr, nullptr, SOCK_CLOEXEC);
        if (c < 0) { if (errno == EINTR) continue; break; }
        std::thread([c] {
...
int main() {
    ...
    fprintf(stderr, "DEV1 Microdroid guest bridge started\n");
    for (;;) pause();
}
