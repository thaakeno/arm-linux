...
# Pinned Vessel UML kernel with SMP/uinput/virtio-gpu and the standard
# VHOST_USER_GPU_SET_SOCKET handoff. This is executable Bionic UML, not a guest
# kernel blob bundled into the multi-GB distro image.
KERNEL_ZIP="$WORK/vessel-uml-smp-arm64.zip"
download_verified \
  "https://github.com/thaakeno/arm-linux/releases/download/vessel-virtio-gpu-kernel-latest/vessel-uml-smp-arm64.zip" \
  "a2d5422b5aab6593bb5a8b8f567414abce9e0665ad93199d110622494af5b0d4" \
  "$KERNEL_ZIP"
mkdir -p "$WORK/kernel"
unzip -q "$KERNEL_ZIP" -d "$WORK/kernel"
...
# The network helpers from the same Bionic UML project are tiny runtime pieces;
# the distro disk itself stays outside the APK in Download/LinuxPC.
download_verified \
  "https://github.com/zalexdev/linux-um-arm64/releases/download/prebuilt-20260816/umnet" \
  "a6e1af3986d354aa9268a6dce665d5c08ca761da2c79ca7dc5709b12121a5694" \
  "$OUT/libvessel_umnet.so"
download_verified \
  "https://github.com/zalexdev/linux-um-arm64/releases/download/prebuilt-20260816/passt" \
  "122efb3aa39e2069e7bddd5791b5ab04b54f06a69d89a22dd70c50834d18e75f" \
...
wanted={'angle-android','virglrenderer-android'}
found={}
for stanza in text.split('\n\n'):
    ...
    if p in wanted:
        found[p]=(fields.get('Filename'),fields.get('SHA256'),fields.get('Version','unknown'))
for p in sorted(wanted):
    if p not in found or not all(found[p][:2]): raise SystemExit(f'missing {p} in Termux package index')
    fn,sha,ver=found[p]
    (out/f'{p}.txt').write_text(f'{fn}\n{sha}\n{ver}\n')
PY
...
[ -n "$ANGLE_DIR" ] || { echo "ANGLE Vulkan directory missing from package" >&2; exit 4; }
...
cp -L "$VIRGL_REAL" "$OUT/libvessel_virglrenderer.so"
cp -L "$EPOXY_REAL" "$OUT/libvessel_epoxy.so"
...
sed -e "s/@VIRGL_MAJOR_VERSION@/$VMAJ/g" -e "s/@VIRGL_MINOR_VERSION@/$VMIN/g" -e "s/@VIRGL_MICRO_VERSION@/$VMIC/g" \
  "$VIRGL_SRC/src/virgl-version.h.meson" > "$INC/virgl-version.h"
...
# Build rust-vmm vhost-device-gpu directly for Android/Bionic. We intentionally
# expose only the classic VirGL features implemented end-to-end by Vessel;
# guest Venus/resource-blob support stays disabled until implemented properly.
...
(
  ...
  cargo build --locked --release --target aarch64-linux-android \
    -p vhost-device-gpu --no-default-features --features backend-virgl
)
...
{
  echo "protocol=39"
  echo "runtime=v39-self-contained-dmabuf-r2"
...
echo "[vessel-native] runtime bundle ready"
ls -lh "$OUT"
file "$OUT"/*.so
