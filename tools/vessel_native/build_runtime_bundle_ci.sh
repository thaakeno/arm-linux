...
# P39 immutable base: UML kernel/stub + networking helpers + ANGLE/VirGL +
# the native virtio-input backend. The GPU executable itself is deliberately
# rebuilt later by rebuild_vhost_gpu_ahb.sh from the current P39 bridge source.
KERNEL_WORK="$WORK/kernel-build"
VESSEL_KERNEL_WORK="$KERNEL_WORK" NDK="$NDK" JOBS="${JOBS:-4}" \
  bash "$ROOT/tools/vessel_native/kernel/build_uml_smp_android.sh"
...
download_verified "https://github.com/zalexdev/linux-um-arm64/releases/download/prebuilt-20260816/umnet" \
  "a6e1af3986d354aa9268a6dce665d5c08ca761da2c79ca7dc5709b12121a5694" "$OUT/libvessel_umnet.so"
download_verified "https://github.com/zalexdev/linux-um-arm64/releases/download/prebuilt-20260816/passt" \
  "122efb3aa39e2069e7bddd5791b5ab04b54f06a69d89a22dd70c50834d18e75f" "$OUT/libvessel_passt.so"
chmod 0755 "$OUT/libvessel_umnet.so" "$OUT/libvessel_passt.so"

# Build-time package source only. Vessel has no Termux runtime dependency.
...
download_verified \
  ...
  "56170f8caa1bb642a2624b649e3bcca095ec2834814e5c308efc8a85a709e4ce" "$VIRGL_TAR"
...
cargo build --release --target aarch64-linux-android --target-dir "$INPUT_TARGET" \
  --manifest-path "$ROOT/tools/vessel_native/vhost-device-vessel-input/Cargo.toml"
...
# Keep the pinned vhost source tree ready. The current AHardwareBuffer patch and
# executable are built only by rebuild_vhost_gpu_ahb.sh, avoiding old DMA-BUF generations.
...
echo "[vessel-native] immutable P39 base ready"
ls -lh "$OUT"
file "$OUT"/*.so
