...
UA="Vessel-BounceQuest-AssetBuilder/3.0"
...
def slab(c,h,mat,accent=None):
    ...
    if accent is not None:
        # Thin painted fascia like the target image, without changing collision dimensions.
        x,y,z=c;X,Y,Z=h
...
def ramp(c,h,rise,mat,arrow=True):
    ...
    if arrow:
        # Raised yellow chevrons, physically part of the authored ramp mesh.
        for dz in (-.18,.18):
            ytop=low+(high-low)*((Z-dz)/(2*Z))+.018
            w=.54
...
def banner(x,y,z,w,h,mat):
    quad((x-w,y+h,z),(x+w,y+h,z),(x+w,y-h,z),(x-w,y-h,z),(0,0,1),mat,0,(1,1))

def course():
    # Cohesive playable course: broad start, ramp, offset platforms, spike trench,
    # narrow bridge and a portal plaza. These dimensions mirror Level.h/physics.
    platforms=[
        ((0,-.42,2.0),(5.8,.42,3.6),0,1),
        ((.7,.10,-3.0),(4.0,.40,1.8),0,1),
        ((-2.8,.55,-6.2),(2.0,.42,1.6),1,0),
        ((1.8,.72,-6.7),(2.6,.42,1.7),0,1),
        ((4.1,1.02,-9.5),(1.7,.40,1.5),1,0),
        ((.8,1.25,-9.7),(2.1,.35,1.1),0,1),
        ((-2.0,1.55,-11.7),(1.6,.34,1.0),0,0),
        ((1.4,1.85,-13.5),(1.7,.34,1.0),0,1),
        ((4.1,2.10,-15.5),(1.6,.34,1.0),1,0),
        ((1.7,2.35,-17.2),(1.8,.34,.90),0,1),
        ((-1.0,2.62,-19.0),(1.7,.34,.90),0,0),
        ((-3.2,2.90,-21.0),(1.6,.34,1.0),1,1),
        ((-.5,3.15,-22.9),(2.0,.34,1.0),0,0),
        ((1.0,3.50,-25.8),(4.2,.42,2.1),0,1),
    ]
    for c,h,m,a in platforms:slab(c,h,m,1 if a else None)

    # Thick start arena curbs/walls, intentionally framing the camera rather than floating.
    ...
    ramp((.15,.05,-.85),(1.65,.10,1.35),.70,2,True)
    ramp((-.70,.88,-8.0),(1.25,.10,1.10),.62,2,True)

    # Bounce pad: layered base, emissive center and corner bolts.
    box((2.15,.04,1.05),(1.10,.11,1.05),4);box((2.15,.16,1.05),(.88,.025,.80),8)
    for sx in (-.83,.83):
        for sz in (-.75,.75):cylinder((2.15+sx,.21,1.05+sz),.055,.035,3,12)

    # Spike trench is now unmistakably visible in the same location used by physics.
    box((0.0,1.08,-10.0),(2.55,.10,.72),4)
    for row,z in enumerate((-9.70,-10.05,-10.40)):
        for i in range(7):spike((-2.15+i*.72,1.18,z),.24,.92,6)

    # Narrow wooden bridge section, with metal posts and a believable drop on both sides.
    for i in range(7):
        box((1.7,2.72,-17.1-i*.55),(1.02,.08,.24),18)
    for z in (-16.9,-18.0,-19.1):
        cylinder((.58,3.10,z),.055,.42,3);cylinder((2.82,3.10,z),.055,.42,3)

    # Portal dais and stone-like arch blocks around the glowing ring.
    slab((1.0,3.48,-25.8),(4.2,.42,2.1),0,1)
    torus((1.0,4.85,-27.0),1.28,.13,9,300)
    for i in range(13):
        a=math.pi*(i/12.0);cx=1.0+math.cos(a)*1.64;cy=4.85+math.sin(a)*1.64
        box((cx,cy,-27.08),(.25,.25,.34),5)
    # portal center disc is thin emissive geometry, not just a ring
    cylinder((1.0,4.85,-27.02),1.05,.025,9,48)

    # Collectibles follow the gameplay route and remain large/readable like the target.
    ...
    # Hero ball is stored once at unit size and transformed by physics in the vertex shader.
    sphere((0,0,0),1.0,10,1,56,28)
    # Player-centered environment shell for mountains/sky; no clear-color void can show through.
...
course()
# Real CC0 prop silhouettes instead of pretending every obstacle is a cube.
